...
- `item edit ITEM [--title T] [--description D] [--price P] [--condition C] [--attr k=v...] [--image F...]` overlays only the flags passed onto the account's own listing (`PUT /api/v3/items/{hash}`, same multipart shape, JSON body when no images) and refuses foreign listings (exit 2). `--attr` resolves the listing's leaf in the create tree (recorded: detail APIs expose only the root taxonomy, so leaf id is not recoverable from the item for `--attr` on this leaf); image-only edits skip category resolution. No change flags is a usage error (exit 2). Verified live 2026-09-15 on the throwaway above: title+price edit, picture replacement, then delete; one older sofa listing (`vjrewv00elzk`) remained.
### user / me
